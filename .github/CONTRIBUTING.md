...
runtime in order to build packer with the Triton plugin.
...
## Setting up Triton plugin for dev

With Go installed, you can already `go get` the Triton plugin and `make dev` in
...
1. Download the Triton plugin source (and its dependencies) by running
   `go get github.com/hashicorp/packer-plugin-triton`. This will download the source to
   `$GOPATH/src/github.com/hashicorp/packer-plugin-triton`.

2. When working on the Triton plugin, first `cd $GOPATH/src/github.com/hashicorp/packer-plugin-triton`
   so you can run `make dev` and easily access other files. `make dev` will build the packer-plugin-triton binary and install it under `$HOME/.packer.d/plugins/`.

3. Make your changes to the Triton plugin source. You can run `make dev` to build and install locally, and `make test` to run unit tests.
   Any compilation errors will be shown when the binaries are rebuilding. If you don't have `make` you can simply run `go build -o packer-plugin-triton` from the project root, and `mv packer-plugin-triton ~/.packer.d/plugins/packer-plugin-triton` to install the plugin.

4. After building the Triton plugin successfully, use the latest version of Packer to build a machine and verify your changes. In the [example folder](https://github.com/hashicorp/packer-plugin-triton/blob/main/example) we provide a basic template. Comment out the `packer {}` block to force Packer use the development binary installed in the previous step.
...
need to [fork the Triton plugin](https://github.com/hashicorp/packer-plugin-triton#fork-destination-box), push your
...
# Develop a patch.
git push https://github.com/myuser/packer-plugin-triton f-my-feature
```
...
3. One of Triton plugin's core team members will look over your contribution and
...
The easiest way to work on a fork is to set it as a remote of the the Triton plugin
project. After following the steps in "Setting up Go to work on the Triton plugin":

1. Navigate to the code:

   `cd $GOPATH/src/github.com/hashicorp/packer-plugin-triton`
...
2. Add the remote by running:
   ...
   `git remote add myuser https://github.com/myuser/packer-plugin-triton.git`
...
Please only apply the minimal vendor changes to get your PR to work. The Triton plugin
...
If the Triton Plugin has [acceptance tests](https://en.wikipedia.org/wiki/Acceptance_testing), these probably have some requirements such as environment variables to be set for API tokens and keys. Each test should error and tell you what are missing, so those are not documented here.
